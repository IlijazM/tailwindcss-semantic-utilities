...
describe('toColorArray', () => {
  it('returns single variable repeated for each step when input has no color-* prefix', () => {
    ...
    expect(result).toEqual(Array(TAILWIND_COLORS_SHADES.length).fill('white'));
  });

  it('expands a variable with prefix "color-" and suffix "-*"', () => {
    const result = toColorArray('color-indigo-*');

    expect(result).toEqual(TAILWIND_COLORS_SHADES.map((step) => `var(--color-indigo-${step})`));
  });
  ...
  it('removes suffix ")" and generates correct output', () => {
    ...
    expect(result).toEqual(TAILWIND_COLORS_SHADES.map((step) => `var(--color-sky-500-${step})`));
  });

  it('handles suffix "-*)"', () => {
    const result = toColorArray('color-rose-*)');

    expect(result).toEqual(TAILWIND_COLORS_SHADES.map((step) => `var(--color-rose-${step})`));
  });

  it('handles suffix "-*"', () => {
    const result = toColorArray('color-teal-*');

    expect(result).toEqual(TAILWIND_COLORS_SHADES.map((step) => `var(--color-teal-${step})`));
  });

  it('handles mixed and invalid inputs gracefully', () => {
    const result = toColorArray('--some-random-value');

    expect(result).toEqual(Array(TAILWIND_COLORS_SHADES.length).fill('--some-random-value'));
  });
});
